...
describe('API key group table cell', () => {
  ...
  test('renders the remaining ratio ring and a localized Auto ratio when API data uses a nonlocalized string', async () => {
  ...
  test('shows only the temporary Cross-group badge when ratio data is unavailable', async () => {
    const container = document.createElement('div')
    document.body.append(container)
    const root = createRoot(container)
